...
const Proposal = ({ sections = {}, title = '' }) => {
  // Function to map and filter out "N/A" and empty string values
  const mapData = (rawData) => {
    if (!rawData) return [];
    return Object.keys(rawData)
      .filter(question => rawData[question] !== "N/A" && rawData[question] !== '')
      .map(question => ({
        question,
  ...
  const createAndDownloadPDF = () => {
    const docDefinition = {
      content: [
        ...
        { text: 'Proposal Information', style: 'title' },
        ...informationData.map(item => [
          { text: item.question, style: 'question' },
          { text: item.answer, style: 'answer' }
        ]).flat(),
        { text: 'Scientific Review (Synopsis)', style: 'title' },
        ...scientificData.map(item => [
          { text: item.question, style: 'question' },
          { text: item.answer, style: 'answer' }
        ]).flat(),
        { text: 'Ethical Review', style: 'title' },
        ...ethicalData.map(item => [
          { text: item.question, style: 'question' },
          { text: item.answer, style: 'answer' }
        ]).flat(),
        { text: 'Consent', style: 'title' },
      ...
      styles: {
        ...
        title: { fontSize: 14, alignment: 'center', bold: true, margin: [0, 10] },
