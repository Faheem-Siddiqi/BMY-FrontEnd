  ...
  export default function Signup() {
    ...
    const handleSubmit = (e) => {
      ...
      if (password !== confirmPassword) {
        setSignUpError("Password and Confirm Password Doesn't Matches")
     return;
      }

      
      if (!/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}$/.test(confirmPassword)) {
        setSignUpError("Password must have one alpha, upercase,lowercase and number eg:Password@123")
      ...
      if (! /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
         setSignUpError("In-Valid Email Address ")

         return;
       }

       if (! /^[+1-9][0-9]*$/.test(phone)) {
        setSignUpError("In-Valid Phone Number ")

        return;
      }

      console.log('Form submitted:', { email, password, role ,name,phone,confirmPassword});
    };
    return (
      <>
          ...
          <div className="w-full max-w-4xl p-5 md:p-20 shadow-sm bg-iota rounded-box">
            <div className='grid grid-cols-1 lg:grid-cols-2'>
              ...
              <div className="md:p-10 p-5 bg-white rounded-r-lg">
                <h2 className="text-3xl font-bold  font-WorkSans-Regular">Sign Up</h2>
                <p className='text-light font-NunitoSans-Regular text-sm mb-4 mt-1'>Get Started For Free</p>
                <div className=' text-sm text-inputFields'>
                  ...
                  <section className="mb-4">
                    ...
                    <span className="text-xs text-red-600">
                      {!password && requiredError && " * Required"} </span>
                    <input
                      name='password'
                      id='password'
                      type="password"
                  ...
                  <section className="mb-4">
                    ...
                    <span className="text-xs text-red-600">
                      {!confirmPassword && requiredError && " * Required"} </span>
                    <input
